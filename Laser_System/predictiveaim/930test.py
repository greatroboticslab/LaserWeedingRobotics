...
class FlySkyRemoteControl:
    ...
    def __init__(self, parent_system, esp32_ip='192.168.1.104', port=10001):
        ...
        self.swa_active = False  # Moving target mode
        self.swb_active = False  # Static target mode
        ...
        print("SwA: Moving target mode | SwB: Auto static detection mode")
            ...
            try:
                ...
                if data:
                    ...
                    while '\n' in buffer:
                        ...
                        self._process_flysky_data(line.strip())
        ...
        if self.swa_active != self.prev_swa:
            if self.swa_active:
                ...
                if self.swb_active or self.parent.auto_static_mode_enabled:
                    print("SwA overrides SwB - switching to moving target mode")
                    self.parent.auto_static_mode_enabled = False
                    self.parent.stationary_weeds.clear()
                    self.parent.static_targeting.stop_static_targeting()

                # Enable auto targeting for moving weeds
                if not self.parent.targeting_enabled:
                    self.parent.targeting_enabled = True
                    print("SwA: MOVING TARGET MODE ENABLED")
                    print("Targeting moving weeds with trajectory prediction")
            else:
                # Disable if SwB not active
                if not self.swb_active and self.parent.targeting_enabled:
                    self.parent.targeting_enabled = False
                    print("SwA: Moving target mode disabled")
...
class StaticTargetingSystem:
    ...
    def __init__(self, parent_system):
        ...
        self.firing_duration = 10.0  # seconds (5-25)
        self.aiming_duration = 1.0  # Aiming phase duration before firing (0.5-3 seconds)

    def start_static_targeting(self, weed_id, target_position):
        ...
        with self.execution_lock:
            ...
            print(f"[STATIC] Starting targeting for Weed #{weed_id}")
            print(f"[STATIC] Phase 1: AIMING ({self.aiming_duration:.1f}s) - Laser OFF")
            print(f"[STATIC] Phase 2: FIRING ({self.firing_duration:.1f}s) - Laser ON")
...
class EnhancedWeedTargeting:
    def __init__(self, model_path='weed4.pt',
        ...
        self.static_firing_duration = 10.0  # Fire for 10 seconds (5-25 range)
    ...
    def _check_stationary_weeds(self, weed, current_time):
        """Check if weed is stationary and trigger auto static targeting"""
        # Only work when auto static mode is enabled
        if not self.auto_static_mode_enabled:
            # Clean up monitoring record if mode is off
            if weed['weed_id'] in self.stationary_weeds:
                del self.stationary_weeds[weed['weed_id']]
            return
        ...
        # Check if moving - need at least some observation time
        observation_time = current_time - weed.get('first_seen', current_time)

        # Wait at least 0.5 seconds before considering as stationary
        ...
        if is_moving:
            ...
            if weed_id not in self.stationary_weeds:
                ...
                print(
                    f"[AUTO-STATIC] Weed #{weed_id} detected as stationary, monitoring for {self.stationary_timeout:.1f}s...")
            else:
    ...
    def _display_frame(self, frame, current_time):
        ...
        cv2.putText(display_frame,
                    f"Static: Timeout={self.stationary_timeout:.1f}s, "
    ...
    def run(self):
        ...
        print("Operation Mode: OBSERVE -> PREDICT -> DUAL MOTOR AUTONOMOUS EXECUTE")
        print("FlySky Remote Control: SwA=Moving | SwB=Auto-Static | SwC=Manual | SwD=Laser")
